        ...
        (current) => {
          ...
          const nextDebts = current.data
            .map((debt) => {
              ...
              const totalPaid =
                debt.paidAmount + Number(data.data.amountPaid ?? 0);
              const remaining = Number(data.data.remaining ?? 0);
              return {
                ...debt,
                paidAmount: totalPaid,
                remaining,
                status: (data.data.isFullyPaid
                  ? "Paid"
                  : "Partial") as Debt["status"],
                payments: [
                  {
                    id: Date.now(),
          ...
          return {
            ...
            data: nextDebts,
            summary: {
              totalDebt: nextDebts.reduce(
                (sum, debt) => sum + debt.remaining,
                0,
              ),
              unpaidCount: nextDebts.filter((debt) => debt.status !== "Paid")
                .length,
                  ...
                  <>
                    ...
                    <div className="space-y-4">
                          ...
                          {[
                            ...
                            .map((preset) => (
                              <button
                                key={preset.value}
                                onClick={() =>
                                  setPayAmount(String(preset.value))
                                }
                      ...
                      <button
                        onClick={handlePay}
                        disabled={
                          paying || !payAmount || Number(payAmount) <= 0
                        }
