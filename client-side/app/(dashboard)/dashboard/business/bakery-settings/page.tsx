...
export default function BakerySettingsPage() {
  ...
  const [staffRoleFilter, setStaffRoleFilter] = useState<StaffRoleFilter>("Semua");
  ...
  useEffect(() => {
    ...
    const loadStaff = async () => {
      try {
        ...
        const payload = (await response.json().catch(() => ({}))) as StaffApiResponse;
        if (!response.ok || !active) return;

        const members = (payload.data?.members ?? [])
          .filter((member) => ["Admin", "Cashier", "Staff"].includes(member.role))
          .map((member) => ({
            userId: member.userId,
            name: member.name,
            role: member.role,
  ...
  const updateStaffSetting = (userId: number, patch: Partial<EditableStaff>) => {
  ...
  const removeHoliday = (date: string) => {
    setHolidayEntries((current) => current.filter((entry) => entry.date !== date));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Gagal menyimpan bakery settings",
      );
    } finally {
      setIsSaving(false);
    }
    ...
    <div className="mx-auto max-w-md space-y-4 bg-[#f7ede2] px-4 py-5 text-[#2f1e13]">
      ...
      {!isOwner ? (
        <div className="rounded-[22px] border border-[#e9cbb6] bg-[#f9e8dc] px-4 py-3 text-sm text-[#a45731]">
          🔒 Halaman ini hanya bisa diubah oleh owner. Role lain hanya dapat melihat data.
        </div>
      ) : null}
        ...
        <div className="space-y-3 px-4 py-4">
          <div className="grid gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              {(["Semua", "Admin", "Cashier", "Staff"] as StaffRoleFilter[]).map(
                (role) => (
                  <button
                    key={role}
                    type="button"
          ...
          <div className="flex items-center justify-between rounded-[18px] border border-[#e2d1c3] bg-[#fbf4ed] px-4 py-3">
            <div>
              <p className="text-sm font-bold">📦 {filteredStaffSettings.length} pegawai tampil</p>
              <p className="text-xs text-[#b58872]">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-xs text-[#b58872]">
                        {entry.isActive ? "Kontrak aktif" : "Pegawai nonaktif"}
                      </p>
                    </div>
          ...
          <div className="flex items-center justify-between rounded-[20px] border border-[#efcfb3] bg-[#fff0df] px-4 py-3">
            <div>
              <p className="text-sm font-bold text-[#9f5129]">💰 Total Gaji Bulan Ini</p>
              <p className="text-xs text-[#c07c54]">Akumulasi pegawai aktif yang tampil.</p>
            </div>
      ...
      <section className="overflow-hidden rounded-[24px] border border-[#ddcbbb] bg-[#f4e9dc] shadow-[0_16px_30px_-26px_rgba(52,31,20,0.35)]">
        <div className="flex items-center justify-between border-b border-[#e8d6c8] px-4 py-3">
          <div>
            <h2 className="text-lg font-bold">🧾 Biaya Operasional Bulanan</h2>
            <p className="text-xs text-[#b58872]">{currentMonthExpenseLabel}</p>
          </div>
          <button
            type="button"
            disabled={!isOwner}
      ...
      <section className="overflow-hidden rounded-[24px] border border-[#ddcbbb] bg-[#f4e9dc] shadow-[0_16px_30px_-26px_rgba(52,31,20,0.35)]">
        <div className="flex items-center justify-between border-b border-[#e8d6c8] px-4 py-3">
          <div>
            ...
            <p className="text-xs text-[#b58872]">
              {currentMonthHolidayCount} hari libur aktif. Order baru ditutup di hari itu.
            </p>
          </div>
        </div>
        <div className="space-y-4 px-4 py-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-2">
                      ...
                      {(entry.label || "").trim().length > 0 ? (
                        <span className="text-xs text-[#8a6047]">{entry.label}</span>
                      ) : null}
                    </div>
                  </div>
                  <button
