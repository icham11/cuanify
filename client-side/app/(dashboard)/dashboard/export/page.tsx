...
function buildBakeryExportRows(orders: BakeryExportOrder[]) {
  return orders.map((order) => {
    ...
    return {
      ...
      "No. HP Customer": order.customerPhone || "",
      "Tanggal Kirim": order.deliveryDate || "",
      "Jam / Sesi Kirim": order.deliverySlot || "",
      "Status Order": order.orderStatus || "",
      "Status Pembayaran": order.paymentStatus || "",
      "Daftar Produk": items,
      "Alamat Pengiriman": addresses,
      "Harga Dasar": Number(order.basePrice || 0),
      "Total Tambahan (Add-On)": Number(order.addOnTotal || 0),
      "Biaya Ongkir": Number(order.deliveryFee || 0),
      "Adjustment Manual": Number(order.manualAdjustment || 0),
      "Total Harga": Number(order.totalPrice || 0),
      "Jumlah DP": Number(order.downPaymentAmount || 0),
      "Sisa Tagihan": Number(order.remainingBalance || 0),
      "Kurir Pengiriman": order.shippingQuote?.provider || "",
      "Layanan Kurir": order.shippingQuote?.courierServiceName || "",
      "Estimasi Harga Kurir": Number(order.shippingQuote?.price || 0),
      "Waktu Estimasi Tiba": order.shippingQuote?.eta || "",
      "Jarak Pengiriman (Km)": Number(order.shippingQuote?.distanceKm || 0),
