...
export default async function DashboardLayout({
  ...
  try {
    ...
    console.warn(
      "[DashboardLayout] No session found or invalid, redirecting to /login",
      error,
    );
    redirect("/login");
  }
    ...
    <DashboardClientLayout>
      <div className="relative min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top,#f8efe6_0%,#f2eae1_42%,#eadfd4_100%)] md:h-screen md:overflow-hidden">
        <div className="relative z-10 flex min-h-screen gap-2 px-2 py-2 md:h-full md:min-h-0 md:gap-3 md:px-3 md:py-3">
          <aside className="custom-scrollbar sticky top-4 hidden h-[calc(100vh-2rem)] min-h-[calc(100vh-2rem)] w-64 shrink-0 self-start flex-col overflow-y-auto rounded-[30px] border border-(--crumbella-border) bg-background px-4 py-5 shadow-[0_18px_38px_-28px_rgba(30,18,10,0.42)] md:flex lg:w-72 lg:px-5 xl:w-80 xl:px-6">
