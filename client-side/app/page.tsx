...
export default function LandingPage() {
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute bottom-0 left-1/2 hidden h-72 w-72 -translate-x-1/2 rounded-full bg-[#f9bd1f]/18 blur-3xl sm:block" />
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-6xl flex-col px-4 pb-10 pt-4 sm:px-6 lg:px-8">
        <main className="mt-4 grid flex-1 items-stretch gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <section
            className={`rounded-4xl border border-[#ffd8b7] bg-[linear-gradient(135deg,#173a7a_0%,#243b5a_45%,#f26a21_100%)] p-6 text-white shadow-[0_28px_60px_-35px_rgba(23,58,122,0.95)] transition-all duration-700 sm:p-9 ${"translate-y-0 opacity-100"}`}
          >
            <div className="-mx-1 mb-5 hidden overflow-hidden rounded-2xl border border-white/20 bg-white/5 sm:-mx-2 md:block">
              <Image
            ...
            {QUICK_ROUTES.map((item, index) => {
              ...
              return (
                <Link
                  ...
                  prefetch={false}
                  className={`group flex h-full flex-col rounded-3xl border border-[#ffd8b7] bg-white/92 p-5 shadow-[0_18px_40px_-30px_rgba(23,58,122,0.8)] transition-all duration-700 hover:-translate-y-0.5 hover:shadow-[0_22px_44px_-30px_rgba(23,58,122,0.9)] ${"translate-y-0 opacity-100"}`}
        ...
        <section
          className="mt-6 rounded-3xl border border-[#dbe2ea] bg-white/90 p-5 shadow-[0_14px_28px_-22px_rgba(23,58,122,0.8)] backdrop-blur-sm transition-all duration-700 translate-y-0 opacity-100 sm:p-6"
