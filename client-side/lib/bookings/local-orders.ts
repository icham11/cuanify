  ...
  return {
    ...
    deliveryDate:
      typeof order.deliveryDate === "string"
        ...
        : "",
    deliverySlot:
      typeof order.deliverySlot === "string" ? order.deliverySlot : "",
    items: Array.isArray(order.items)
      ? order.items.map((item) => {
          const source = item as Record<string, unknown>;
          return {
            productName:
              typeof source.productName === "string" ? source.productName : "",
    ...
    deliveryAddresses: Array.isArray(order.deliveryAddresses)
      ? order.deliveryAddresses.map((address) => {
          ...
          return {
            ...
            addressLine:
              typeof source.addressLine === "string" ? source.addressLine : "",
          };
        })
      : [],
    paymentStatus:
    ...
    shippingQuote:
      order.shippingQuote && typeof order.shippingQuote === "object"
        ? {
            provider:
              typeof (order.shippingQuote as Record<string, unknown>)
                .provider === "string"
            ...
            courierServiceName:
              typeof (order.shippingQuote as Record<string, unknown>)
                .courierServiceName === "string"
                ? ((order.shippingQuote as Record<string, unknown>)
                    .courierServiceName as string)
                : "",
            price: asNumber(
              (order.shippingQuote as Record<string, unknown>).price,
            ),
            eta:
              typeof (order.shippingQuote as Record<string, unknown>).eta ===
              "string"
                ? ((order.shippingQuote as Record<string, unknown>)
                    .eta as string)
                : "",
            distanceKm: asNumber(
              (order.shippingQuote as Record<string, unknown>).distanceKm,
            ),
          }
        : null,
    shipment:
      order.shipment && typeof order.shipment === "object"
        ? {
            trackingNumber:
              typeof (order.shipment as Record<string, unknown>)
                .trackingNumber === "string"
                ? ((order.shipment as Record<string, unknown>)
                    .trackingNumber as string)
                : "",
            status:
              typeof (order.shipment as Record<string, unknown>).status ===
              "string"
    ...
    simulations:
      order.simulations && typeof order.simulations === "object"
        ? {
            productionWhatsappSent: Boolean(
              (order.simulations as Record<string, unknown>)
...
export function parseLocalBakeryOrders(
...
export function summarizeLocalBakeryOrders(
  ...
  const nowDate =
    normalizeDateInput(todayIsoDate ?? "") ?? toIsoDateString(new Date());
  const tomorrowBase = parseSafeDate(nowDate);
  const tomorrowIso = tomorrowBase
    ? toIsoDateString(addDays(tomorrowBase, 1))
    : "";
  ...
  return orders.reduce<LocalBakerySummary>(
    (acc, order) => {
      ...
      const normalizedDeliveryDate = normalizeDateInput(
        order.deliveryDate ?? "",
      );
      ...
      if (
        ...
        normalizedDeliveryDate < nowDate &&
        isOpenOrder
      ) {
